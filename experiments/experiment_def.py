...
def run_expreminent(look_forward, hidden_size, batch_size, epochs, dropout, dataset):
    ...
    train_x = np.reshape(train_x, (-1, look_forward, train_x.shape[1]))
    test_x = np.reshape(test_x, (-1, look_forward, test_x.shape[1]))

    model = create_model(hidden_size=hidden_size, look_forward=look_forward, dropout=dropout)
    model = train(model, train_x, train_y, test_x, test_y, batch_size=batch_size, epochs=epochs)
...
run_expreminent(look_forward=1, hidden_size=10, batch_size=200, epochs=10000, dropout=0.3, dataset=dataset)
